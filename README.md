...
Agent Security Proxy は、外部エージェントや子エージェントから届く入力を、連携先の AI エージェント実行環境へ渡す前に検査する単体動作のセキュリティプロキシです。
...
- hash chain 付き append-only JSONL audit log
- kill-switch file
...
command mode は、`agent chat -Q --source agent-security-proxy ... -q <prompt>` の形で backend CLI を呼びます。`toolsets` はデフォルトで空です。外部エージェント入口では、必要な tool だけを backend 側で明示的に許可してください。
...
backend の応答は、呼び出し元へ返す前に検査されます。output guard は次の内容を block または review stop します。
...
```bash
python3 -m py_compile proxy.py tests/test_proxy.py scripts/init_runtime_config.py scripts/smoke_test.py
```
